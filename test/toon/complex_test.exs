defmodule Toon.ComplexTest do
  ...
  alias Toon.Fixtures.Complex
  alias Toon.Fixtures.Complex.{Address, Budget, Department, Employee, Organisation, Project}
  ...
  defp encode!(data, opts \\ []), do: Toon.encode!(data, opts)
  defp decode!(toon, opts \\ []), do: Toon.decode!(toon, opts)
  ...
  describe "normalize" do
    ...
    test "whole-number float score normalises to integer (9.0 → 9)" do
      # Toon.Utils.normalize does NOT change floats; the integer conversion
  ...
  describe "partial decode — individual fields" do
    test "inline tags array" do
      {:ok, r} = Toon.decode("tags[2]: leadership,backend")
  ...
  describe "error paths" do
    test "tabular header row count too low raises" do
      ...
      assert_raise Toon.DecodeError, fn -> decode!(toon) end
    end
    ...
    test "inline array length mismatch raises" do
      assert_raise Toon.DecodeError, fn -> decode!("tags[3]: a,b") end
    end
    ...
    test "tab indentation in strict mode raises" do
      toon = "head:\n\tname: Alice"
      assert_raise Toon.DecodeError, fn -> decode!(toon, strict: true) end
    end

    test "non-multiple-of-2 indentation in strict mode raises" do
      toon = "head:\n   name: Alice"
      assert_raise Toon.DecodeError, fn -> decode!(toon, strict: true, indent_size: 2) end
    end
  end
end
