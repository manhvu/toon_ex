defmodule Toon.Roundtrip.FullTest do
  @moduledoc """
  ...
  Every test follows the same pattern — the assertion is always:
    decoded == Toon.Utils.normalize(input)
  ...
  defp rt(value, opts \\ []) do
    encoded = Toon.encode!(value, opts)
    ...
    {:ok, decoded} = Toon.decode(encoded, decode_opts)
  ...
  describe "primitives" do
    ...
    # The is_finite boundary is tested in Toon.Utils.NormalizeTest.
  ...
  describe "decode correctness — values not from encode" do
    ...
    test "spaces around comma delimiter in inline array" do
      {:ok, r} = Toon.decode("x[2]: a , b")
      assert r["x"] == ["a", "b"]
    end

    test "empty token in inline array" do
      {:ok, r} = Toon.decode("x[3]: a,,c")
      assert r["x"] == ["a", "", "c"]
    end

    test "tab-delimited inline array" do
      {:ok, r} = Toon.decode("x[3\t]: 1\t2\t3")
      assert r["x"] == [1, 2, 3]
    end
  end
  ...
  describe "decode errors" do
    ...
    test "tabular row count mismatch" do
      assert_raise Toon.DecodeError, fn ->
        Toon.decode!("rows[3]{a}:\n  1\n  2")
      end
    end

    test "tabular column count mismatch" do
      assert_raise Toon.DecodeError, fn ->
        Toon.decode!("rows[1]{a,b}:\n  1,2,3")
      end
    end

    test "invalid escape in quoted string" do
      assert_raise Toon.DecodeError, fn -> Toon.decode!(~s(x: "\\q")) end
    end

    test "unterminated quoted string" do
      assert_raise Toon.DecodeError, fn -> Toon.decode!(~s(x: "no end)) end
    end
    ...
    test "non-existent atom with keys: :atoms!" do
      assert_raise Toon.DecodeError, fn ->
        Toon.decode!("zz_unique_nonexistent_key_xyz_999: 1", keys: :atoms!)
      end
    end
  end
end
