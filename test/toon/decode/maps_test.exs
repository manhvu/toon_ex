defmodule Toon.Decode.MapsTest do
  ...
  describe "basic maps" do
    ...
    test "multiple key-values" do
      {:ok, result} = Toon.decode("name: Alice\nage: 30")
      assert result == %{"name" => "Alice", "age" => 30}
    end

    test "empty value produces empty map" do
      {:ok, result} = Toon.decode("nested: ")
      assert result["nested"] == %{}
    end

    test "key with underscore" do
      assert {:ok, %{"first_name" => "Bob"}} = Toon.decode("first_name: Bob")
    end

    test "key with dot is a literal dotted key by default" do
      assert {:ok, %{"user.name" => "Ada"}} = Toon.decode("user.name: Ada")
    end

    test "numeric-looking key is treated as a string" do
      assert {:ok, %{"123" => "val"}} = Toon.decode(~s("123": val))
    end
  end
  ...
  describe "quoted keys" do
    test "quoted key with spaces" do
      assert {:ok, %{"full name" => "Alice"}} = Toon.decode(~s("full name": Alice))
    end

    test "quoted key with special characters" do
      assert {:ok, %{"a-b" => 1}} = Toon.decode(~s("a-b": 1))
    end

    test "quoted key with escaped quote" do
      assert {:ok, %{"say \"hi\"" => "ok"}} = Toon.decode(~s("say \\"hi\\"": ok))
    end

    test "quoted dotted key is NOT expanded with expand_paths: safe" do
      {:ok, result} = Toon.decode(~s("a.b": 1), expand_paths: "safe")
      assert result == %{"a.b" => 1}
    end
  end
  ...
  describe "keys: :atoms" do
    test "converts string keys to atoms" do
      assert {:ok, %{name: "Alice"}} = Toon.decode("name: Alice", keys: :atoms)
    end

    test "nested keys also become atoms" do
      {:ok, result} = Toon.decode("user:\n  name: Bob", keys: :atoms)
  ...
  describe "keys: :atoms!" do
    test "existing atom succeeds" do
      ...
      assert {:ok, %{name: _}} = Toon.decode("name: Alice", keys: :atoms!)
    end

    test "non-existing atom raises" do
      assert_raise Toon.DecodeError, fn ->
        Toon.decode!("zz_nonexistent_atom_xyz_abc_999: 1", keys: :atoms!)
      end
    end
  end
  ...
  describe "nested objects" do
    test "one level deep" do
      toon = "user:\n  name: Bob"
      assert {:ok, %{"user" => %{"name" => "Bob"}}} = Toon.decode(toon)
    end

    test "two levels deep" do
      toon = "a:\n  b:\n    c: 1"
      assert {:ok, %{"a" => %{"b" => %{"c" => 1}}}} = Toon.decode(toon)
    end
    ...
    test "empty nested object" do
      toon = "meta:"
      assert {:ok, %{"meta" => %{}}} = Toon.decode(toon)
    end
  end
  ...
  describe "strict mode indentation" do
    test "tab in indentation raises" do
      assert_raise Toon.DecodeError, ~r/Tab/, fn ->
        Toon.decode!("parent:\n\tchild: 1", strict: true)
      end
    end
    ...
    test "blank line inside nested block raises in strict mode" do
      ...
      assert_raise Toon.DecodeError, fn ->
        Toon.decode!(toon_arr, strict: true)
      end
    end
